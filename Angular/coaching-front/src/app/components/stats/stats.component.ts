...
export class StatsComponent implements OnInit {
  ...
  obtenerTitularidades(id?:number) : number{
    var contador = 0;
  ...
  obtenerPartidosJugados(id?:number): number{
    var contador = 0;
  ...
  obtenerMinutosJugados(id?:number): number{
    var contador = 0;

    this.actions.forEach(a=>{
      if(a.player.id == id)

        if(a.type == 'CAMBIO_ENTRA'){
          contador = contador + 90 - a.min/60;
        }else if(a.type == 'CAMBIO_SALE'){
          contador = contador - 90 + a.min/60;
        }else if(a.type == 'INICIAL'){
  ...
  public ACTIONS =[ {name : "FALTA_A_FAVOR", id : 1, description : "Falta a favor"},
